const asyncHandler=require("express-async-handler");
const alumni=require("../model/alumniModel");
const get_all_alumni = asyncHandler(async (req, res) => {
  const { name, city, batch, department, profession, search, page, limit } = req.query;
  let filter = { user_id: req.user.id };

  if (name) filter.name = { $regex: name, $options: "i" };
  if (city) filter.city = { $regex: city, $options: "i" };
  if (batch) filter.batch = batch;
  if (department) filter.department = { $regex: department, $options: "i" };
  if (profession) filter.profession = { $regex: profession, $options: "i" };

  if (search) {
    filter.$or = [
      { name: { $regex: search, $options: "i" } },
      { city: { $regex: search, $options: "i" } },
      { department: { $regex: search, $options: "i" } },
      { profession: { $regex: search, $options: "i" } },
      { batch: { $regex: search, $options: "i" } },
    ];
  }
  const pageNumber = parseInt(page) || 1;
  const pageSize = parseInt(limit) || 10;
  const skip = (pageNumber - 1) * pageSize;

  const total = await alumni.countDocuments(filter); 
  const alumniData = await alumni
    .find(filter)
    .skip(skip)
    .limit(pageSize)
    .sort({ name: 1 });

  res.json({
    page: pageNumber,
    pageSize,
    total,
    totalPages: Math.ceil(total / pageSize),
    data: alumniData,
  });
});
const get_alumni=asyncHandler(async(req,res)=>{
    const alumniData = await alumni.findOne({
        _id: req.params.id,
        user_id: req.user.id
    });
    if(!alumniData){
        res.status(404);
        throw new Error("Alumni not found!");
    }
    res.status(200).json(alumniData);
});

const add_alumni=asyncHandler(async(req,res)=>{
    const{name,
        image,
        profession,
        roll,
        email,
        phone,
        city,
        currentLocation,
        hometown,
        company,
        workSector,
        university,
        department,
        batch,
        school,
        college,
        bloodGroup,
        parentsName,
        interests,
        bio,
        linkedin,
        github,
        facebook,} =req.body;
    if(!name || !email || !batch || !department || !profession || !company){
        res.status(400);
        throw new Error("all fields are mandatory!");
    }
    const alumniData=await alumni.create({
    user_id:req.user.id,
        name,
        image,
        profession,
        roll,
        email,
        phone,
        city,
        currentLocation,
        hometown,
        company,
        workSector,
        university,
        department,
        batch,
        school,
        college,
        bloodGroup,
        parentsName,
        interests,
        bio,
        linkedin,
        github,
        facebook});
        
    res.status(201).json(alumniData);
});
const update_Alumni = asyncHandler(async (req, res) => {
    const alumniData= await alumni.findById(req.params.id);

    if (!alumniData) {
        res.status(404);
        throw new Error("Not found");
    }

    if (alumniData.user_id.toString() !== req.user.id) {
        res.status(403);
        throw new Error("Unauthorized");
    }

    const updated = await alumni.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(updated);
});
const delete_Alumni = asyncHandler(async (req, res) => {
    const alumniData= await alumni.findById(req.params.id);

    if (!alumniData) {
        res.status(404);
        throw new Error("Not found");
    }

    if (alumniData.user_id.toString() !== req.user.id) {
        res.status(403);
        throw new Error("Unauthorized");
    }

    await alumni.findByIdAndDelete(req.params.id);
    res.json({ message: "Deleted" });
});

module.exports={get_alumni,add_alumni,get_all_alumni,update_Alumni ,delete_Alumni};